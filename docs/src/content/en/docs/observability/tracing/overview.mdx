...
`DefaultObservabilityConfig` provides Mastra's recommended defaults:
...
`DefaultObservabilityConfig` is equivalent to this more verbose configuration:
...
export const mastra = new Mastra({
  observability: new Observability({
    configs: {
      default: new DefaultObservabilityConfig(),
      langfuse: {
...
```ts title="src/mastra/index.ts" showLineNumbers copy
import { DefaultExporter, CloudExporter, Observability } from "@mastra/observability";
