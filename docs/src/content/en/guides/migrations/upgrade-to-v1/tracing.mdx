...
`DefaultObservabilityConfig` automatically includes the `DefaultExporter`, `CloudExporter`, and `SensitiveDataFilter` processor. See the [observability tracing documentation](/docs/v1/observability/tracing/overview) for full configuration options.
...
3. Use `DefaultObservabilityConfig` for quick setup, or custom `configs:` for more control
...
Key changes:
1. Install `@mastra/observability` package
2. Import `Observability` and `DefaultObservabilityConfig` from `@mastra/observability`
3. Use `DefaultObservabilityConfig` in the `configs` object

:::note[Legacy Support]
The `default: { enabled: true }` pattern is still supported for backwards compatibility:
...
However, `DefaultObservabilityConfig` is the recommended approach as it's more explicit about what's being configured.
:::
...
To migrate, wrap your configuration with `new Observability()` and use `DefaultObservabilityConfig`:
