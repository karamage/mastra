...
Pre-configured observability instance with Mastra's recommended defaults. Use this for the simplest setup:
...
`DefaultObservabilityConfig` provides:
  ...
  props={[
    {
      ...
      description: "Legacy option to enable default configuration. Prefer using DefaultObservabilityConfig in configs instead.",
      required: false,
    },
    {
      name: "configs",
      type: "Record<string, Omit<ObservabilityInstanceConfig, 'name'> | ObservabilityInstance>",
      description: "Named observability instance configurations or pre-instantiated instances (including DefaultObservabilityConfig)",
      required: false,
    },
    {
      name: "configSelector",
